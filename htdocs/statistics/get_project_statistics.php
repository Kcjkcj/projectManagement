...
class User_Statistics {
    ...
    public function getAllProjectData() {
        $sql = "SELECT DISTINCT
            ...
            WHERE 
                p.version = (SELECT MAX(version) FROM project WHERE id = p.id)
            ...
            if ($dependencyType === 'task') {
                foreach ($allTasks as $task) {
                    if ($task['id'] == $dependencyId) {
                        foreach ($task['subtasks'] as $depSubtask) {
                ...
                foreach ($allTasks as $task) {
                    if (isset($task['subtasks']["subtask_$dependencyId"])) {
                        $depTime = $this->calculateSubtaskCompletionTime($task['subtasks']["subtask_$dependencyId"], $completionTimes, $allTasks);
                        $maxDependencyTime = max($maxDependencyTime, $depTime);
                        break;
                    }
                }
    ...
    public function displayStatistics($data) {
        ...
        foreach ($data as &$project) {
            $dependencyResult = $this->set_dependency([$project['project_name']=>$project]);
            $total_allocated_days = $dependencyResult['maxProjectTime'];
            echo "<h3>프로젝트: " . htmlspecialchars($project['project_name']) . "</h3>";
            
            echo "<table border='1'>";
            echo "<tr><th>태스크</th><th>서브태스크</th><th>할당된 멤버</th><th>할당된 일수</th><th>실제 소요 일수</th><th>누적 소요 일수</th><th>성공 여부</th></tr>";
            ...
            foreach ($project['tasks'] as &$task) {
                foreach ($task['subtasks'] as &$subtask) {
                    ...
                    $assigned_member_username = $this->getMemberName( $subtask['subtask_id']);
                    ...
                    echo "<td>" . htmlspecialchars($assigned_member_username) . "</td>";
...
$DB = new Database();
$user_statistics = new User_Statistics($DB);
